...
async function routeAdmin(ctx: any, route: string, method: string, query: AnyRow, body: AnyRow) {
  ...
  if(body.serviceCategory!==undefined&&!['SHORT_STAY','HOME','OFFICE','COMMON_AREAS','OTHER'].includes(body.serviceCategory))throw new ApiError('Invalid service category');
  const portfolio=route.match(/^admin\/clients\/(\d+)\/properties(?:\/(\d+))?$/);
  if(portfolio){
    if(ctx.appUser.role!=='ADMIN')throw new ApiError('Forbidden',403);
    ...
    if(manager.account_type!=='PROPERTY_MANAGER'||managerUser.role!=='PROPERTY_MANAGER'||!managerUser.active)throw new ApiError('Active property manager required');
    if(method==='GET'&&!portfolio[2])return {objectIds:(await result(service.from('st_manager_properties').select('object_id').eq('manager_client_id',manager.id))).map((r:AnyRow)=>r.object_id)};
    const objectId=Number(portfolio[2]||body.objectId);
    if(!Number.isSafeInteger(objectId)||objectId<=0)throw new ApiError('Choose a property');
    await one(service.from('st_objects').select('id').eq('id',objectId).single());
    if(method==='POST'&&!portfolio[2]){await result(service.from('st_manager_properties').upsert({manager_client_id:manager.id,object_id:objectId,assigned_by_user_id:ctx.appUser.id},{onConflict:'manager_client_id,object_id'}));return {ok:true};}
    if(method==='DELETE'&&portfolio[2]){await result(service.from('st_manager_properties').delete().eq('manager_client_id',manager.id).eq('object_id',objectId));return {ok:true};}
  ...
  if (jobRoute) {
    ...
    if(action === 'cancel' && method === 'POST') return {job: (await hydrateJobs([await jobCommand(ctx,id,'cancel',body)]))[0]};
