...
(()=>{
  ...
  function render(){
    ...
    const el=document.createElement('section');el.className='st-dispatch-console';el.innerHTML=`<div class="st-dispatch-head"><div><div class="st-eyebrow">DISPATCH CONTROL</div><h2>Live assignment desk</h2><p>Real API assignment and rescue actions — no simulated success.</p></div><div class="st-dispatch-actions"><button class="st-dispatch-btn" data-action="refresh">↻ Recalculate</button><button class="st-dispatch-btn danger" data-action="rescue">⚡ Rescue queue</button></div></div><div class="st-dispatch-metrics"><div><span>Open</span><strong data-m="open">—</strong></div><div><span>Unassigned</span><strong data-m="unassigned">—</strong></div><div><span>Rebalance</span><strong data-m="rebalance">—</strong></div><div><span>Urgent</span><strong data-m="urgent">—</strong></div></div><div class="st-dispatch-body"><div class="st-dispatch-list" data-list><div class="st-dispatch-empty">Loading live dispatch data…</div></div><aside class="st-dispatch-side"><strong>Operator checklist</strong><button data-quick="unassigned">Review unassigned</button><button data-quick="risk">Review at-risk</button><button data-quick="rescue">Open rescue candidates</button><button data-quick="all">Show full board</button></aside></div>`;
    p.prepend(el);el.querySelector('[data-action="refresh"]').onclick=refresh;el.querySelector('[data-action="rescue"]').onclick=()=>document.querySelector('[data-filter="rescue"]')?.click()||toast('Rescue queue filter is ready on the board','success');el.querySelectorAll('[data-quick]').forEach(b=>b.onclick=()=>document.querySelector(`[data-filter="${b.dataset.quick}"]`)?.click()||document.querySelector('[data-filter="all"]')?.click());refresh();
  }
