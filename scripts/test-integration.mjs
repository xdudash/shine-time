...
try{
 ...
 const {booking}=await api(3,'client/bookings','POST',{objectId:1,serviceDate:date,startTime:'10:00',idempotencyKey:crypto.randomUUID()});
 assert.equal(booking.status,'ACCEPTED');
 const cleaner=Number(booking.assigned_cleaner_id);
 const path=`cleaner/jobs/${booking.id}`;
 for(const status of ['EN_ROUTE','ARRIVED','CLEANING'])await api(cleaner,`${path}/status`,'POST',{status,idempotencyKey:crypto.randomUUID()});
 ...
 await assert.rejects(api(cleaner,`${path}/complete`,'POST',{idempotencyKey:crypto.randomUUID()}),/photo/i);
 ...
 await api(cleaner,`${path}/complete`,'POST',{idempotencyKey:crypto.randomUUID()});
 await assert.rejects(api(3,'admin/settings'),/403|Forbidden|permission/i);
 const report={realAuthRoles:5,booking:true,signedStorageUpload:true,idempotentFinalize:true,completion:true,productionTouched:false};
