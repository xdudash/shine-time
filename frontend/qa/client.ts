...
document.getElementById('refresh-qa')?.addEventListener('click',()=>setTimeout(live,10000));
...
export async function api<T>(route:string,{method='GET',body={},query={}}:any={}):Promise<T>{await new Promise(r=>setTimeout(r,150));let data:any;
 ...
 else if(route==='admin/settlements/monthly'){const rows=Array.from({length:24},(_,i)=>({id:100+i,object_name:properties[i%2].name,service_date:serviceToday().slice(0,7)+'-'+String(i+1).padStart(2,'0'),chargedCents:query.side==='CLIENT'?5900:2700,paidCents:monthlyPaid.get(query.side+'-'+(100+i))||0})).map(j=>({...j,dueCents:j.chargedCents-j.paidCents}));data={groups:[{id:query.side==='CLIENT'?21:7,name:query.side==='CLIENT'?'QA Client Company':'QA Cleaner',count:rows.length,chargedCents:rows.reduce((a,j)=>a+j.chargedCents,0),paidCents:rows.reduce((a,j)=>a+j.paidCents,0),dueCents:rows.reduce((a,j)=>a+j.dueCents,0)}],jobs:query.partyId?rows:[],hasMore:false}}
 else if(route==='admin/settlements/batch'){for(const j of body.items)monthlyPaid.set(body.side+'-'+j.jobId,j.amountCents);data={count:body.items.length,amountCents:body.items.reduce((a:number,j:any)=>a+j.amountCents,0)}}
