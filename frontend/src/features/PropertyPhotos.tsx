...
function PhotoCard({photo,route,editable,reload}:{photo:Photo;route:string;editable:boolean;reload:()=>Promise<void>}){
 ...
 useEffect(()=>{let active=true;setSrc('');setError('');api<{dataUrl:string}>(`${route}/${photo.id}`).then(r=>{if(active)setSrc(r.dataUrl)}).catch(e=>{if(active)setError(e.message)});return()=>{active=false}},[route,photo.id]);
...
export function PropertyPhotos({objectId,jobId,editable=false}:{objectId?:number;jobId?:number;editable?:boolean}){
 ...
 const reload=async()=>{try{const r=await api<{photos:Photo[]}>(route);setPhotos(r.photos);setPage(p=>Math.min(p,Math.max(0,Math.ceil(r.photos.length/12)-1)));setError('')}catch(e){setPhotos([]);setError((e as Error).message)}finally{setLoading(false)}};
 useEffect(()=>{let live=true;setPage(0);setError('');setPhotos([]);setLoading(true);api<{photos:Photo[]}>(route).then(r=>{if(live){setPhotos(r.photos);setPage(p=>Math.min(p,Math.max(0,Math.ceil(r.photos.length/12)-1)));setError('')}}).catch(e=>{if(live)setError(e.message)}).finally(()=>{if(live)setLoading(false)});return()=>{live=false}},[route]);
